...
%             plot( wayPoints,0);
        ...
        function delegate = createDelegate(o)
            if (~isempty(o.delegate))
                ...
                %use estimated pdf
                %x=wayPoints;
%                 yy=o.model.logPdfEstimate2(x,10000000);
%                 plot(x,yy);
%                 hold on;
%                 plot(x,o.model.logPdf(x),'r')
%                 figure;
                %weights = (1/(length(wayPoints)))*ones(1,length(wayPoints));
                %lp=log((max(wayPoints)-min(wayPoints))/length(wayPoints))+yy;
                
                %lp=log((max(wayPoints)-min(wayPoints))/length(wayPoints))+o.model.logPdf(x);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                
                    
                %we filter zero weights out because they will make some
                %elements of o.c be zeros (note o.c = [o.w ;1./exp(o.u)])
%                 j=1;
%                 for i=1:length(x)
%                 if exp(lp(i))>0
%                     xx(j)=x(i);
...
%                 modelT.sigma = sigma;
%                 modelT.T = o.model.T;
%                 prices = modelT.simulatePricePaths(500000,1);
...
%                
%                  prices = o.model.simulatePricePaths(1500000,1);
%                 logprice=log(prices)-log(o.model.S0);
%                 figure();
%                 histogram(logprice)
%                 scenarios = prices(:,end); 
%                 figure();
...
%             constraintVector = vertcat( positivityVector, 0, -minPayoff);    
        ...
        function [mark,hedgingPort] = markToMarket( o, port )
            % Compute the super and sub hedging price for the instrument
            wayPoints = unique(horzcat(0, o.getWayPoints(), port.getWaypoints() ));
            wayPoints = [wayPoints];
            nWayPoints = length( wayPoints );
            ...
            maxWayPoints=max(wayPoints);
            wayPointPayoffsMax = zeros(1, nInstruments*2+1);
            wayPointPayoffsMaxExtended = zeros(1, nInstruments*2+1);
            
            for i=1:nInstruments
                instrument = o.instruments{i};
                pp = instrument.payoff( maxWayPoints );
                wayPointPayoffsMax(:,i)=pp';
                wayPointPayoffsMax(:,i+nInstruments)=-pp';
                ppp = instrument.payoff( maxWayPoints+5 );                
                wayPointPayoffsMaxExtended(:,i)=ppp';
                wayPointPayoffsMaxExtended(:,i+nInstruments)=-ppp';
            end
            
            diffWayPointPayoffs=wayPointPayoffsMax-wayPointPayoffsMaxExtended;
            
                        
            minPayoff = port.payoff( wayPoints )';
            minPayoffMax=port.payoff( maxWayPoints )';
            minPayoffMaxExtended = port.payoff( maxWayPoints+5 )';
            
            % Positive quantity constraint
            positivityMatrix = -horzcat(eye(2*nInstruments), zeros(2*nInstruments,1));
            positivityVector = zeros(2*nInstruments,1);
            
            constraintMatrix = vertcat( positivityMatrix, costVector, -wayPointPayoffs,diffWayPointPayoffs);
            constraintVector = vertcat( positivityVector, 0, minPayoff, minPayoffMaxExtended-minPayoffMax);
%             constraintMatrix = vertcat( positivityMatrix, costVector, -wayPointPayoffs  );
%             constraintVector = vertcat( positivityVector, 0, -minPayoff);    
            ...
            %constraintMatrix
            %constraintVector
            [x, mark, exitFlag] = linprog( objective, constraintMatrix, constraintVector,[],[],[],ub );

            hedgingPort=Portfolio();
            quantities=x(1:nInstruments)-x(nInstruments+1:nInstruments+nInstruments);
            hedgingPort.add(quantities,o.instruments);
