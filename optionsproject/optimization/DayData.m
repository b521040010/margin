...
%                     end                    
        ...
        function daysToExpiry = getDaysToExpiry(o)
            ...
            %number of bussiness days from start to end but not including
            %the ending point
            daysToExpiry = days252bus(datestr(o.dateNum),datestr(expiryDateNum));
        end
        
        function T = getT(o)
            T = o.getDaysToExpiry()/252;
        end
   ...
   %          m.mu=(mu/m.T)+0.5*m.sigma^2; 
        ...
        function m = varianceGammaModelHist(dd)
            m = VarianceGammaModel2();
            m.T = dd.getT();
            m.S0 = dd.getSpot();
             fileName = strcat( '../SPXFuturesAndOptions/','SPXdataOpen','.xlsx');
             histD = HistoricalData(fileName);
             ...
             maturity=datestr(dd.dateNumMaturity,24);
             [prices dates]=selectTheIntervals(histD,startingDate,maturity);
             [sigma,v] = calibrateVarianceGamma(histD,prices)
%             m.alpha=alpha*m.T*252;
%             m.beta=beta;
            m.m = 0;
            m.theta = 0;
            m.sigma = sigma;
